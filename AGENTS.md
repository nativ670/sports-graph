...
- Players table: frame (int), track_id (int, unique per match), team ("home" | "away"), x_m, y_m (float, non-null)
- Coordinates: meters, origin at pitch center, x along the length, y up
- Match metadata (stored beside the tables): fps, pitch_length_m, pitch_width_m
- Positions may fall slightly outside the pitch; bounds checks use the metadata plus a margin
- No row for a player means not observed in that frame
- Ball: separate table (frame, x_m, y_m), same coordinates
